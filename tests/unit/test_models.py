...
def test_pile_state_observation_transitions_between_unknown_top_seen_and_empty_confirmed():
    ...
    pile.mark_empty_confirmed(source="sim_camera", frame_id="frame-2", observed_at_utc="2026-03-27T12:00:00+00:00")
    ...
    assert pile.observation.observed_at_utc == "2026-03-27T12:00:00+00:00"

    pile.mark_unknown(source="verification", frame_id="frame-3", observed_at_utc="2026-03-27T12:01:00+00:00")
    assert pile.observation.state == PileObservationState.UNKNOWN
    assert pile.discovered is False
    assert pile.has_known_count() is False
    assert pile.observation.frame_id == "frame-3"


def test_pile_state_can_report_observation_staleness():
    pile = PileState(pile_id=PileId(0, 0), role=PileRole.FEEDER, capacity=10)
    pile.mark_top_card_seen(
        "Snapcaster Mage",
        confidence=0.9,
        source="sim_camera",
        observed_at_utc="2026-03-27T12:00:00+00:00",
    )

    assert pile.observation_is_stale("2026-03-27T12:00:05+00:00", max_age_seconds=10.0) is False
    assert pile.observation_is_stale("2026-03-27T12:00:20+00:00", max_age_seconds=10.0) is True
